...
class SkillTooltip {
    constructor() {
        ...
        this.tooltipEdit = document.getElementById('tooltipEdit');
        
        this.tooltipTextarea = document.getElementById('tooltipTextarea');
        this.tooltipSave = document.getElementById('tooltipSave');
        this.tooltipCancel = document.getElementById('tooltipCancel');
        
        this.currentSkill = null;
        this.isEditing = false;
        
        // Store skill descriptions in localStorage
        this.descriptions = JSON.parse(localStorage.getItem('skillDescriptions')) || this.getDefaultDescriptions();
    ...
    getDefaultDescriptions() {
        return {
            'Custom Editing PC Build': 'Built high-performance workstations optimized for 4K/8K video editing with custom cooling and storage solutions.',
            'Edit Suite Build-Out': 'Complete edit bay design including acoustics, lighting, ergonomics, and workflow optimization.',
            'Network Attached Storage': 'Enterprise-level NAS systems with RAID configurations for secure, high-speed media storage.',
            '10+ Gigabit Connections': 'Ultra-fast network infrastructure for real-time collaborative editing and instant file transfers.',
            'Adobe Premiere Pro Training': 'Comprehensive training on advanced editing techniques, workflow optimization, and custom shortcuts.',
            'DIT (Digital Imaging Technician)': 'On-set media management, quality control, and immediate delivery of footage for dailies.',
            'Data Wrangling': 'Secure backup and organization of all media files with proper naming conventions and metadata.',
            'On-set Rush Editing': 'Live editing during production to provide immediate feedback and rough cuts for director review.',
            'Remote Rush Editing': 'Real-time editing via secure connections while production continues at distant locations.',
            'VAMP - Video Asset Management Platform': 'Custom-built application for organizing, searching, and sharing video assets across multiple drives and locations.',
            '🚀 AI Multicam Premiere Plugin': 'AI-powered plugin that analyzes footage and suggests optimal multicam angles based on action, focus, and composition.'
        };
    }
    ...
    init() {
        ...
        document.querySelectorAll('.skill-tag').forEach(tag => {
            tag.addEventListener('mouseenter', (e) => this.showTooltip(e));
            tag.addEventListener('mouseleave', () => this.hideTooltip());
            tag.addEventListener('mousemove', (e) => this.updateTooltipPosition(e));
        });
        
        // Edit functionality
        this.tooltipDescription.addEventListener('click', () => this.startEditing());
        this.tooltipSave.addEventListener('click', () => this.saveDescription());
        this.tooltipCancel.addEventListener('click', () => this.cancelEditing());
        
        // Prevent tooltip from disappearing when hovering over it
        this.tooltip.addEventListener('mouseenter', () => this.keepTooltipOpen());
    ...
    showTooltip(event) {
        ...
        const skillText = event.target.textContent.trim();
        this.currentSkill = skillText;
        
        this.tooltipTitle.textContent = skillText;
        
        const description = this.descriptions[skillText] || 'Click to add description...';
        this.tooltipDescription.textContent = description;
    ...
    updateTooltipPosition(event) {
        if (!this.tooltip.classList.contains('active')) return;
    ...
    startEditing() {
        ...
        this.isEditing = true;
        this.tooltipEdit.style.display = 'block';
        this.tooltipDescription.style.display = 'none';
        
        const currentDesc = this.descriptions[this.currentSkill] || '';
        this.tooltipTextarea.value = currentDesc;
        this.tooltipTextarea.focus();
    }
    
    saveDescription() {
        if (!this.currentSkill) return;
        
        const description = this.tooltipTextarea.value.trim();
        
        if (description) {
            this.descriptions[this.currentSkill] = description;
            localStorage.setItem('skillDescriptions', JSON.stringify(this.descriptions));
            this.tooltipDescription.textContent = description;
        } else {
            delete this.descriptions[this.currentSkill];
            localStorage.setItem('skillDescriptions', JSON.stringify(this.descriptions));
            this.tooltipDescription.textContent = 'Click to add description...';
        }
    ...
    cancelEditing() {
        this.isEditing = false;
        this.tooltipEdit.style.display = 'none';
        this.tooltipDescription.style.display = 'block';
    }
}
